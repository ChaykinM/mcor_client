<template>
    <div v-if="historyCustomParams.length != 0">
                            ...
                            <v-tooltip location="bottom">
                                <template v-slot:activator="{ props }">
                                    <v-btn color="indigo-darken-4" v-bind="props" class="mx-2">
                                        ...
                                        <EditHistory history_type="custom_params" :history="history"
                                            @save-history="onEditHistory" />
                                    </v-btn>
                                </template>
...
export default {
    ...
    computed: {
        histories() {
            let histories = this.historyCustomParams.filter((history) => {
                return history.name
                    .toString()
                    .toLowerCase()
