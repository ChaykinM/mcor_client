<template>
                        ...
                        <v-tooltip location="bottom">
                            <template v-slot:activator="{ props }">
                                <v-btn color="deep-purple-darken-1" class="mx-2" v-bind="props">
                                    ...
                                    <AddIKTpoint :template="template" :disabled="cadAddPoint" @add-point="onAddPoint" />
                                </v-btn>
                            </template>
                        ...
                        <v-tooltip location="bottom">
                            <template v-slot:activator="{ props }">
                                <v-btn color="indigo-darken-4" class="mx-2" v-bind="props">
                                    ...
                                    <EditIKTpoint :template="template" :point="point" @edit-point="onEditPoint"/>
                                </v-btn>
                            </template>
    ...
    <v-alert v-if="points.length == 0" type="error" color="red-darken-4" variant="flat" class="ma-2" width="100%">
        Нет добавленных обобщённых координат
    </v-alert>

    <v-divider></v-divider>
    <div class="d-flex justify-end" style="width: 100%;">
        <v-btn @click="solveDKT" :disabled="canSolveDKT" color="indigo-darken-2" variant="flat" class="ma-2"> Получить декартовые координаты </v-btn>
    </div>

</template>

<script>
import AddIKTpoint from '@/components/account/Kinematics/AddIKTpoint.vue';
import EditIKTpoint from '@/components/account/Kinematics/EditIKTpoint.vue';
...
export default {
    ...
    emits: ['clearDirectPoint', 'solveDirectTask'],
    data() {
        return {
            T: this.maxTime,
            N: this.pointsNumber, 
    ...
    components: { AddIKTpoint, EditIKTpoint },
    computed: {
        cadAddPoint() {
    ...
    methods: {
        ...
        solveDKT() {
            this.$emit('solveDirectTask', this.points); 
        }
    }
}
</script>